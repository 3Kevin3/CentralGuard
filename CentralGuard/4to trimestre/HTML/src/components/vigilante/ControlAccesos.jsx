...
export default function ControlAccesos() {
  ...
  const cargarAccesos = async () => {
    const { data } = await supabase.from('accesos').select('*').order('id', { ascending: false })
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (editId) {
      await supabase.from('accesos').update({ nombre_visitante: nombre, destino, placa }).eq('id', editId)
      Swal.fire({ icon: 'success', title: '¡Actualizado!', text: 'Acceso modificado', timer: 1500, showConfirmButton: false })
      setEditId(null)
    } else {
      await supabase.from('accesos').insert([{ nombre_visitante: nombre, destino, placa }])
  ...
  return (
    <div className="p-4">
      {/* Encabezado Verde estilo Control Peatonal/Vehicular */}
