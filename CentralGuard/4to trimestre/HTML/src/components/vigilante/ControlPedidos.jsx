...
export default function ControlPedidos({ usuario }) {
  ...
  useEffect(() => { cargarPedidos() }, [idUsuario])
    ...
    <div className="p-3">
      ...
      <table className="table table-hover align-middle bg-white rounded-3 overflow-hidden shadow-sm">
        ...
        <tbody>
          {pedidos.length === 0 ? (
            <tr>
              <td colSpan="6" className="text-center py-4 text-muted">No hay pedidos registrados.</td>
            </tr>
          ) : (
            pedidos.map(p => (
              <tr key={p.id}>
