...
export default function ObjetosPerdidos({ usuario }) {
  ...
  useEffect(() => { cargarObjetos() }, [idUsuario])

  const handleSubmit = async (e) => {
    e.preventDefault()
    if (editId) {
      await supabase.from('objetos_perdidos')
        .update({ articulo, ubicacion_hallazgo: ubicacion, estado, entregado_a: entregadoA })
        .eq('id', editId).eq('idusuario', idUsuario)
      Swal.fire('Actualizado', 'Registro de objeto modificado', 'success')
      setEditId(null)
    } else {
      await supabase.from('objetos_perdidos')
        .insert([{ articulo, ubicacion_hallazgo: ubicacion, estado, entregado_a: entregadoA, idusuario: idUsuario }])
