...
export default function SupervisorPanel({ seccionActiva, onCambiarSeccion }) {
  ...
  const cargar = useCallback(async () => {
    setCargando(true)
    try {
      let resNovedades = await supabase
        .from('novedad')
        .select('*')
        .order('fecha', { ascending: false })

      if (resNovedades.error) {
        resNovedades = await supabase
          .from('novedades')
          .select('*')
          .order('fecha', { ascending: false })
      }

      const [r, v, a, m] = await Promise.all([
        supabase.from('ronda').select('*'),
        supabase.from('vigilantes').select('*'),
        supabase.from('asistencias').select('*'),
        supabase.from('mensajes_internos').select('*').order('fecha', { ascending: true })
      ])
  ...
  const rondasRealizadas = useMemo(() => rondas.filter(r => normalizar(r.estado) === 'completada' || normalizar(r.estado) === 'realizada').length, [rondas])
  const rondasPendientes = useMemo(() => rondas.filter(r => normalizar(r.estado) === 'pendiente' || normalizar(r.estado) === 'asignada').length, [rondas])
  ...
  const agregarNovedadSupervisor = async (e) => {
    ...
    if (error) {
      return Swal.fire('Error', 'No se pudo registrar la novedad: ' + error.message, 'error')
    }
  ...
  const cambiarEstadoNovedad = async (id, nuevoEstado) => {
    ...
    if (error) {
      return Swal.fire('Error', 'No se pudo actualizar el estado: ' + error.message, 'error')
    }

    setNovedades(prev => prev.map(n => (n.idnovedad === id || n.idnovedades === id) ? { ...n, estado: nuevoEstado } : n))
    Swal.fire('Actualizado', `La novedad cambió a estado: ${nuevoEstado}`, 'success')
  }

  const eliminarNovedad = async (id) => {
    const confirm = await Swal.fire({
      title: '¿Eliminar novedad?',
      text: 'Esta acción borrará la novedad permanentemente.',
      icon: 'warning',
    ...
    if (error) {
      return Swal.fire('Error', 'No se pudo eliminar la novedad: ' + error.message, 'error')
    }

    setNovedades(prev => prev.filter(n => n.idnovedad !== id && n.idnovedades !== id))
    Swal.fire('Eliminada', 'La novedad ha sido eliminada.', 'success')
  }
...
const columnasNovedades = useMemo(() => [
    {
      ...
      cell: row => React.createElement('div', null,
        React.createElement('div', { className: 'fw-semibold' }, row.fecha || '—'),
        React.createElement('small', { className: 'text-muted' }, row.hora || '')
      )
    },
    {
      name: 'Vigilante / Emisor',
      selector: row => row.vigilante || row.nombre_vigilante || 'Supervisor',
      sortable: true,
      grow: 1.2
    },
    {
    ...
    },
    {
      ...
      sortable: true,
      grow: 1.2,
      cell: row => React.createElement('select', {
        className: 'form-select form-select-sm',
        value: row.estado || 'Pendiente',
        onChange: (e) => cambiarEstadoNovedad(row.idnovedad || row.idnovedades, e.target.value)
      },
        React.createElement('option', { value: 'Pendiente' }, 'Pendiente'),
        React.createElement('option', { value: 'En proceso' }, 'En proceso'),
        React.createElement('option', { value: 'Resuelta' }, 'Resuelta')
      )
    },
    {
      name: 'Acciones',
      grow: 0.8,
      cell: row => React.createElement('button', {
        className: 'btn btn-outline-danger btn-sm',
        onClick: () => eliminarNovedad(row.idnovedad || row.idnovedades),
        title: 'Eliminar novedad'
      }, '🗑️')
    }
  ], [])
  ...
  const editarMensaje = async (id, textoActual) => {
    ...
    if (nuevoTexto && nuevoTexto.trim() !== textoActual) {
      const { error } = await supabase
        .from('mensajes_internos')
        .update({ mensaje: nuevoTexto.trim() })
        .eq('id', id)
      ...
      Swal.fire('Editado', 'El mensaje ha sido actualizado.', 'success')
    }
  }
...
return (
  <div className="supervisor-content">
    ...
    <div className="supervisor-page-title">
      <div>
        ...
        <h1>
          ...
          {seccionActiva === 'monitoreo' && 'Monitoreo de vigilantes'}
        ...
        <p className="text-muted mb-0">
          ...
          {seccionActiva === 'monitoreo' && 'Consulta la ubicación operativa de los vigilantes.'}
          {seccionActiva === 'novedades' && 'Consulta, filtra y actualiza las novedades registradas.'}
    ...
    {seccionActiva === 'monitoreo' && (
      <section className="panel-section">
        <div className="panel-card">
          <span className="eyebrow"> MONITOREO DE VIGILANTES</span>
          <h3 className="h5 mb-3">Ubicación Operativa en Tiempo Real</h3>

          <div className="vigilantes-grid">
            {vigilantes.length > 0 ? (
              vigilantes.map((v, i) => (
                <div key={i} className="vigilante-card">
                  <div className="vigilante-avatar">
                    {String(v.nombre || 'V').charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <strong className="d-block">{v.nombre || 'Vigilante'}</strong>
                    <span className="location-badge"> {v.ubicacion || 'Sin ubicación'}</span>
                  </div>
                </div>
              ))
            ) : (
              <p className="text-muted mb-0">No hay vigilantes registrados o activos.</p>
            )}
          </div>
        </div>
      </section>
            ...
            <div className="row g-3">
              <div className="col-md-4">
                ...
                <input
                  ...
                  placeholder="Ej: Falla de cámara, Portón averiado..."
                  value={tipoNovedad}
              ...
              <div className="col-md-4">
                ...
                <input
                  type="text"
                  className="form-control"
                  placeholder="Nombre del vigilante o 'Supervisor'"
                  value={vigilanteAsignado}
                  onChange={e => setVigilanteAsignado(e.target.value)}
                />
              </div>
          ...
          <div className="filters-bar mb-3">
            <div>
              ...
              >
                ...
                {listaVigilantesNovedades.map((v, idx) => (
        ...
        <div className="chat-card">
          ...
          <div className="chat-messages">
            {mensajes.map((m) => {
              ...
              const horaEnviado = obtenerHoraFormateada(m.fecha)
                ...
                <div key={m.id} className={`message-row ${esPropio ? 'message-own' : 'message-other'}`}>
                  <div className="message-bubble">
                    ...
                    <div className="message-footer">
                      <span className="message-time"> {horaEnviado || 'Reciente'}</span>
                      {esPropio && (
                        <div className="chat-actions">
                          <button className="chat-btn-action" onClick={() => editarMensaje(m.id, texto)}>
                             Editar
                          </button>
                          <button className="chat-btn-action" onClick={() => eliminarMensaje(m.id)}>
                             Eliminar
                          </button>
                        </div>
                      )}
