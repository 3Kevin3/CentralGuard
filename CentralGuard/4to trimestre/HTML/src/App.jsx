...
import ValidacionRegistros, { ContadorPendientes } from './components/roles/ValidacionRegistros'
import MisEquipos from './components/vigilante/MisEquipos'
import RegistroRecorrido from './components/vigilante/RegistroRecorrido'
...
function App() {
  ...
  const [subSeccionAdmin, setSubSeccionAdmin] = useState('administracion')
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // 3. El administrador debe haber aceptado el registro
      if (data.estado_registro === 'pendiente') {
        Swal.fire({
          title: 'Cuenta en revisión',
          text: 'Necesitas ser aceptado por el administrador para iniciar sesión. Vuelve a intentarlo cuando tu registro sea aprobado.',
          icon: 'info',
          confirmButtonColor: '#0d6efd'
        })
        return
      }
      if (data.estado_registro === 'rechazado') {
        Swal.fire({
          title: 'Registro rechazado',
          text: 'El administrador no aprobó tu registro. Comunícate con él si crees que es un error.',
          icon: 'error',
          confirmButtonColor: '#0d6efd'
        })
        return
  ...
  const handleRegistro = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: 'Necesitas ser aceptado por el administrador para iniciar sesión. Cuando apruebe tu cuenta podrás ingresar con tu correo y contraseña.',
        icon: 'info',
        confirmButtonColor: '#0d6efd'
      })
    } catch (error) {
      ...
      <div className="dashboard-shell">
        <aside className="sidebar">
          ...
          <nav className="side-nav">
            {rol === 'vigilante' && <>
              ...
              <button className={subSeccionVigilante==='equipos'?'active':''} onClick={()=>setSubSeccionVigilante('equipos')}><i className="bi bi-shield-check me-2"></i><span>Mis equipos</span></button>
            </>}
            {rol === 'supervisor' && <>
              ...
              <button className={subSeccionSupervisor==='turnos'?'active':''} onClick={()=>setSubSeccionSupervisor('turnos')}><i className="bi bi-calendar-check me-2"></i><span>Programar turnos</span></button>
              ...
              <button className={subSeccionSupervisor==='advertencias'?'active':''} onClick={()=>setSubSeccionSupervisor('advertencias')}><i className="bi bi-exclamation-octagon me-2"></i><span>Advertencias</span></button>
              <button className={subSeccionSupervisor==='activos'?'active':''} onClick={()=>setSubSeccionSupervisor('activos')}><i className="bi bi-tools me-2"></i><span>Gestión de activos</span></button>
              <button className={subSeccionSupervisor==='objetos'?'active':''} onClick={()=>setSubSeccionSupervisor('objetos')}><i className="bi bi-box-seam me-2"></i><span>Objetos perdidos</span></button>
              <button className={subSeccionSupervisor==='pedidos'?'active':''} onClick={()=>setSubSeccionSupervisor('pedidos')}><i className="bi bi-bag-check me-2"></i><span>Pedidos y deliveries</span></button>
            </>}
            {rol === 'admin' && <>
              <button className={subSeccionAdmin==='administracion'?'active':''} onClick={()=>setSubSeccionAdmin('administracion')}><i className="bi bi-gear-fill me-2"></i><span>Administración</span></button>
              <button className={subSeccionAdmin==='validacion'?'active':''} onClick={()=>setSubSeccionAdmin('validacion')}><i className="bi bi-person-check-fill me-2"></i><span>Validación de registros</span><ContadorPendientes /></button>
            </>}
          </nav>
        ...
        <section className="dashboard-main">
          ...
          <main className="dashboard-content">
            {rol === 'admin' && (subSeccionAdmin === 'validacion' ? <ValidacionRegistros /> : <AdminPanel />)}
            {rol === 'supervisor' && <SupervisorPanel usuario={usuarioLogueado} seccionActiva={subSeccionSupervisor} onCambiarSeccion={setSubSeccionSupervisor} />}
            {rol === 'vigilante' && (
              <div className="panel-card">
                ...
                {subSeccionVigilante === 'novedades' && (<><RegistroNovedades usuario={usuarioLogueado} /><RegistroRecorrido usuario={usuarioLogueado} /></>)}
                ...
                {subSeccionVigilante === 'equipos' && <MisEquipos usuario={usuarioLogueado} />}
              </div>
            )}
          </main>
        </section>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content p-4 border-0 shadow-lg rounded-4">
              <div className="d-flex justify-content-between align-items-center mb-3">
                ...
                <button onClick={() => { setMostrarModalLogin(false); setVerPassword(false) }} className="btn-close" />
              </div>
        ...
        <div className="modal show d-block" style={{ backgroundColor: 'rgba(0,0,0,.6)' }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content p-4 border-0 shadow-lg rounded-4">
              ...
              <div className="alert alert-info small py-2 rounded-3">
                <i className="bi bi-info-circle-fill me-1"></i>Tu cuenta deberá ser aceptada por el administrador antes de poder iniciar sesión.
              </div>
