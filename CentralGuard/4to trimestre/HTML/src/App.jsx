  ...
  const handleLogin = async (e) => {
    ...
    try {
      const { data: usuariosData, error: usuarioError } = await supabase
        .from('usuarios')
        .select('*')
        .eq('usuario', emailInput.trim().toLowerCase())
        ...
        <aside className="sidebar">
          ...
          <nav className="side-nav">
            ...
            {rol === 'supervisor' && <>
              ...
              <button className={subSeccionSupervisor==='monitoreo'?'active':''} onClick={()=>setSubSeccionSupervisor('monitoreo')}><i className="bi bi-geo-alt me-2"></i><span>Monitoreo de vigilantes</span></button>
