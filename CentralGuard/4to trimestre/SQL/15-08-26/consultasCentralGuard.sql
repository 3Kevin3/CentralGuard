...
-- 2. Novedades reportadas con su tipo, vigilante y puesto de vigilancia
select n.idnovedad, tn.nombre_tipo as tipo_novedad, n.hora_reporte, n.estado, u.nombre, u.apellido, pv.nombre_identificador
from novedades n
...
inner join puesto_vigilancia pv on t.idpuesto_vigilancia = pv.idpuesto_vigilancia;
 ...
 -- 3. Equipos actualmente asignados (sin devolver) con el usuario y su rol
...
-- 4. Pedidos entregados con su registro de acceso, persona que lo trajo y turno
...
inner join documento d on emp.iddocumento = d.iddocumento;
 ...
 -- 7. Vehículos con la placa, el conductor y su tipo de persona (residente, visitante, etc.)
...
where ev.puntaje_numerico > (select avg(puntaje_numerico) from evaluacion_desempeño);
 ...
 -- 3. Puestos de vigilancia con más objetos perdidos hallados
select pv.nombre_identificador, c.nombre_conjunto, tp.nombre_tipo AS tipo_puesto, COUNT(op.idobjeto_perdido) AS total_objetos
from objeto_perdido op
inner join puesto_vigilancia pv on op.idpuesto_vigilancia = pv.idpuesto_vigilancia
inner join conjunto c on pv.idconjunto = c.idconjunto
inner join tipo_puesto tp on pv.idtipo_puesto = tp.idtipo_puesto
group by pv.nombre_identificador, c.nombre_conjunto, tp.nombre_tipo
having count(op.idobjeto_perdido) = (select max(cnt) from (select COUNT(*) cnt from objeto_perdido group by idpuesto_vigilancia) sub);
...
where pv.idpuesto_vigilancia in (select t.idpuesto_vigilancia from turno t join novedades n on n.idturno = t.idturno group by t.idpuesto_vigilancia having count(n.idnovedad) > 1);
...
-- 9. Empleados con más de una advertencia del mismo motivo (reincidencia)
